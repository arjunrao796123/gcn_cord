...
for filename in os.listdir(folderName):
    ...
    with open(fullPath, 'rb') as document:
        sent_tokenize.append(document.read().decode('utf-8').replace(stop_re, '').split())

w2v = FastText(sent_tokenize, size=200, window=5, min_count=2, workers=4,sg=1)
...
class Graph:
    ...
    def _get_text_features(self, data):
        ...
        features = np.append(features,np.array(w2v[word]))
        return features
